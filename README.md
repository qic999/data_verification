...
- an official-annotation availability matrix for every dataset adapted so far,
  distinguishing direct visible-2D/3D/projected-envelope fields from boxes
  derived by the project and recording whether absolute metric scale exists;
- a top-level audit statistics table;
- a separate full 3D-evidence result column that keeps
  physical 3D verification separate from projection-only checks;
...
object-case statistics, and Pass / Review / Rejected galleries. DSLR and iPhone
use the released COLMAP cameras. 360 Panorama uses the released
azimuth/elevation maps and scanner poses; it is not projected as a pinhole
camera.
...
The displayed policy is the 3D-evidence v3 rule. It first checks whether
each training 2D box comes from the visible image or from the 3D box. IoU and normalized
center thresholds are decision metrics only for same-semantic amodal
projection checks. Visible-to-amodal comparisons use asymmetric visible
containment; their IoU and center offset are diagnostic only. Five additional
checks use depth, backprojected 3D points, raw pose validity, static multi-view
agreement, and free-space evidence when those signals exist. Missing evidence
is reported as unavailable rather than passed. Depth selected only by a 2D-box
proxy, and transparent/reflective-object depth, is diagnostic only. See
`DATA_FILTERING_RULES.md` for the canonical policy and dataset semantic profiles,
and `3D_EVIDENCE_AUDIT_V3_REPORT.md` for the current full-run results.

The full v3 run covers every standardized JSON scene and object frame in
Structured3D, 3D-FRONT, HSSD, ABO, ShapeNet, HOI4D, HOPE Image, HOPE Video,
SUN RGB-D, Synscapes, and ATEK. ScanNet++ keeps its separate complete official
mesh/camera audit. A dataset without the required standardized metadata or
independent evidence is shown as **Not run**, not as passed.

Counts and galleries generated by older cross-semantic IoU/center rules are
historical evidence until those datasets are re-audited with v3. They must not
be used as a deletion list solely because an old IoU or center threshold fired.
...
python scripts/visualize_scannetpp_official_multisensor_website.py
python scripts/update_scannetpp_website.py
...
For an incremental ScanNet++ refresh, skip the first command. The ScanNet++
updater keeps the three sensor views separate.
