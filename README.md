...
The displayed policy is the semantic-aware v2 rule. It first checks whether
...
containment; their IoU and center offset are diagnostic only. See
`../DATA_FILTERING_RULES.md` in the SpatialEncoder repository for the canonical
policy and dataset semantic profiles.

Counts and galleries generated by older cross-semantic IoU/center rules are
historical evidence until those datasets are re-audited with v2. They must not
