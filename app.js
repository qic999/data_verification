(() => {
  ...
  const elements = {
    ...
    th3dEvidence: document.querySelector("#th-3d-evidence"),
    ...
    commentGuideKicker: document.querySelector("#comment-guide-kicker"),
    commentGuideTitle: document.querySelector("#comment-guide-title"),
    commentGuideIntro: document.querySelector("#comment-guide-intro"),
    commentGuideStep1: document.querySelector("#comment-guide-step-1"),
    commentGuideStep2: document.querySelector("#comment-guide-step-2"),
    commentGuideStep3: document.querySelector("#comment-guide-step-3"),
    commentGuideStep4: document.querySelector("#comment-guide-step-4"),
    commentGuideNote: document.querySelector("#comment-guide-note"),
  ...
  function format3dEvidence(audit) {
    if (!audit || audit.status === "not_run") {
      return localized("Not run", "尚未运行", true);
    }
    const counts = audit.counts || {};
    const pass = Number(counts["3d_evidence_pass"] || 0);
  ...
  function renderStaticLanguage() {
    const staticCopy = [
      ...
      [elements.th3dEvidence, "3D evidence audit", "3D 证据审查"],
      ...
      [elements.commentGuideKicker, "CASE FEEDBACK", "CASE 反馈"],
      [elements.commentGuideTitle, "Have a question or an extra comment about a case?", "对某个 case 有疑问或补充意见？"],
      [elements.commentGuideIntro, "Each visualized case has its own saved comment thread.", "每个可视化 case 都有单独且长期保存的评论区。"],
      [elements.commentGuideStep1, "<strong>Open the case</strong> by clicking its image or card.", "点击图片或卡片，<strong>打开 case</strong>。"],
      [elements.commentGuideStep2, "Scroll to <strong>Case comments</strong> below the enlarged image.", "向下滚动到大图下方的 <strong>Case comments</strong>。"],
      [elements.commentGuideStep3, "Click <strong>Start a comment thread on GitHub</strong>, sign in, write your comment, and submit the issue. If a thread already exists, use <strong>View or add comment on GitHub</strong>.", "点击 <strong>Start a comment thread on GitHub</strong>，登录后填写评论并提交 issue；如果已有评论区，请点击 <strong>View or add comment on GitHub</strong>。"],
      [elements.commentGuideStep4, "Return to this page and click <strong>Refresh comments</strong> to display the saved comment.", "返回本页面，点击 <strong>Refresh comments</strong>，即可显示已保存的评论。"],
      [elements.commentGuideNote, "Comments are saved in this repository's GitHub Issues and remain available to other reviewers. A GitHub account is required to post, and comments are public.", "评论保存在本仓库的 GitHub Issues 中，其他审核者之后仍可查看。发布需要 GitHub 账号，评论内容公开可见。"],
  ...
  function renderStats() {
      ...
      .map((dataset) => {
        ...
        return `
          <tr data-dataset="${dataset.id}" tabindex="0" aria-label="View ${dataset.name}">
            ...
            <td>${format3dEvidence(dataset.evidenceAudit)}</td>
    ...
    const totals = data.datasets.reduce(
      (result, dataset) => {
        ...
        const evidenceCounts = (dataset.evidenceAudit && dataset.evidenceAudit.counts) || {};
        result.evidenceObservations += Number((dataset.evidenceAudit && dataset.evidenceAudit.observations) || 0);
        result.evidencePass += Number(evidenceCounts["3d_evidence_pass"] || 0);
        result.evidenceProjectionOnly += Number(evidenceCounts["projection_only_pass"] || 0);
        result.evidenceReview += Number(evidenceCounts.review || 0);
        result.evidenceHard += Number(evidenceCounts.hard_reject || 0);
  ...
  function renderFacts(dataset, stats) {
    ...
    elements.facts.innerHTML = [
      ...
      ["3D evidence audit", "3D 证据审查", format3dEvidence(dataset.evidenceAudit)],
    ]
      .map(
